	...
	<table class="w-full border-collapse text-sm">
		<thead class="bg-gray-50 text-gray-700">
			<tr>
				...
				<th class="px-4 py-3 text-left font-medium">Khoa</th>
				<th class="px-4 py-3 text-center font-medium">Hành động</th>
			</tr>
		</thead>
		<tbody class="divide-y divide-gray-100">
			{#each data as major, index}
				<tr class="transition-colors hover:bg-gray-50">
					<td class="px-4 py-3 text-center text-gray-600">
						{(pagination.current - 1) * pagination.pageSize + index + 1}
					</td>
					<td class="px-4 py-3 font-mono text-blue-600">{major.code}</td>
					<td class="px-4 py-3">{major.name}</td>
					<td class="px-4 py-3">
						{major.department ? `${major.department.name}` : major.deptId}
					</td>
					<td class="px-4 py-3 text-center">
						<div class="flex justify-center gap-2">
							...
							<button
								...
								on:click={() => handleDelete(major.id)}
								title="Xóa"
							>
