<script>
	...
	import { FileText, Users, FileCheck, Layers } from 'lucide-svelte';
	...
	async function fetchStats() {
		try {
			...
			const [studentsRes, certsRes, certsStatusRes, templatesRes] = await Promise.all([
				axios.get(`${API_URL}/students/count`),
				axios.get(`${API_URL}/certs/count`),
				axios.get(`${API_URL}/certs/stats-by-status`),
				axios.get(`${API_URL}/templates/count`)
			]);

			stats.totalStudents = studentsRes.data.total;
			stats.totalCerts = certsRes.data.total;
			stats.certsByStatus = certsStatusRes.data;
			stats.totalTemplates = templatesRes.data.total;
	...
	async function fetchRecentCerts() {
		try {
			loadingRecent = true;
			const res = await axios.get(`${API_URL}/certs`, { params: { limit: 5, sort: 'desc' } });

			const allCerts = res.data.data || res.data.rows || [];
			recentCerts = allCerts.slice(0, 5);
		} catch (err) {
			console.error(err);
	...
	async function fetchCharts() {
		try {
			// Chứng chỉ theo năm
			const certsYearRes = await axios.get(`${API_URL}/certs/stats-by-year`);
			certsYearData = {
...
<div class="space-y-6 p-6">
	...
	<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
		...
		<div class="flex items-center gap-4 rounded-xl bg-purple-100 p-4">
			<Layers class="h-8 w-8 text-purple-600" />
			<div>
				<p class="text-sm text-gray-600">Tổng template</p>
				<p class="text-xl font-bold">{loadingStats ? '...' : stats.totalTemplates}</p>
			</div>
		</div>
	</div>
				...
				<tbody class="divide-y divide-gray-100">
					{#each recentCerts as cert}
						<tr>
							...
							<td class="px-3 py-2"
								>{cert.gradDate ? new Date(cert.gradDate).toLocaleDateString() : '-'}</td
							>
