<script>
	...
	function handleView(template) {
		dispatch('view', template);
	}
	function handleEdit(template) {
		dispatch('edit', template);
	}
	function handleDelete(id) {
		if (confirm('Bạn có chắc chắn muốn xóa mẫu văn bằng này?')) {
			dispatch('delete', id);
		}
	}
	...
	<table class="w-full border-collapse text-sm">
		<thead class="bg-gray-50 text-gray-700">
			<tr>
				...
				<th class="px-4 py-3 text-left font-medium">Tên mẫu văn bằng</th>
				<th class="px-4 py-3 text-left font-medium">Mô tả</th>
				<th class="px-4 py-3 text-center font-medium">Hành động</th>
			</tr>
		</thead>
		<tbody class="divide-y divide-gray-100">
			{#each data as template, index}
				<tr class="transition-colors hover:bg-gray-50">
					<td class="px-4 py-3 text-center text-gray-600">
						{(pagination.current - 1) * pagination.pageSize + index + 1}
					</td>
					<td class="px-4 py-3 font-mono text-blue-600">{template.name}</td>
					<td class="px-4 py-3">{template.desc}</td>
